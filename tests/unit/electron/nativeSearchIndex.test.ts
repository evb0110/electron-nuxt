...
describe('native search index sidecar', () => {
    ...
    it('persists page text in the binary format consumed by evb-pdf-search', async () => {
        ...
        await persistNativeSearchIndex(pdfPath, index);
        ...
        expect(payload.readUInt32LE(12)).toBe(2);
        expect(payload.readUInt32LE(16)).toBe(2);

        const firstTextOffset = Number(payload.readBigUInt64LE(24 + 8));
        const firstTextLength = Number(payload.readBigUInt64LE(24 + 16));
        const secondRecordOffset = 24 + 24;
        ...
        expect(payload.readUInt32LE(24)).toBe(1);
        expect(payload.readUInt32LE(24 + 4)).toBe('alpha'.length);
