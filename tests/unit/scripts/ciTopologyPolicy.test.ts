...
describe('CI topology policy', () => {
    it('keeps PR feedback bounded and release workflow checks delegated', async () => {
        ...
        expect(packageJson).toContain('"check:static:fast": "pnpm run check:github-actions-syntax && pnpm run check:style-assets');
        ...
        expect(packageJson).toContain('"check:static:assets": "pnpm run check:web-deploy-source"');
