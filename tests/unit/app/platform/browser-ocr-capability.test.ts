...
vi.mock('@app/platform/browser-api/browser-ocr-language-store', () => ({
...
vi.mock('@app/utils/browser-ocr-config', () => ({ getBrowserOcrLanguageBaseUrl: () => '/ocr-langs' }));
...
describe('browser OCR capability', () => {
    ...
    it('prefers a single worker on low-memory devices', async () => {
        const { resolveBrowserOcrWorkerCount } = await import('@app/platform/browser-api/ocr-capability');
    ...
    it('limits in-flight OCR jobs to the adaptive worker count', async () => {
        ...
        const { browserOcrCapability } = await import('@app/platform/browser-api/ocr-capability');
    ...
    it('preinstalls requested languages without starting OCR recognition', async () => {
        ...
        } = await import('@app/platform/browser-api/ocr-capability');
