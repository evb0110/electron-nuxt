...
function createPdfPage(pageNumber: number): PDFPageProxy {
    // The coordinator uses page identity and pageNumber. PDF.js supplies the
    // remaining proxy members when the application loads a real page.
    return {pageNumber} as PDFPageProxy;
}
...
describe('runCoordinatedPdfPageRender', () => {
    ...
    it('allows synchronous renders for different page proxies to overlap', async () => {
        const firstPage = createPdfPage(1);
