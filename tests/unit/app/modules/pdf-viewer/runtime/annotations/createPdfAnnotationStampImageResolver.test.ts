...
const {mockResolveStampImageDataUrl} = vi.hoisted(() => ({
    mockResolveStampImageDataUrl: vi.fn(() => 'data:image/png;base64,stamp'),
}));
vi.mock('@app/modules/pdf-viewer/runtime/annotations/resolvePdfJsStampImageDataUrl', () => ({
    resolvePdfJsStampImageDataUrl: mockResolveStampImageDataUrl,
}));
...
const entity = {
    ...
    modifiedAt: null,
    author: null,
    rect: {left: 0, top: 0, width: 0.2, height: 0.2},
    rotation: 0,
    image: {
        objectNumber: 11,
        generationNumber: 0,
...
describe('createPdfAnnotationStampImageResolver document ownership', () => {
    it('does not resolve or cache a stamp after the document changes during page parsing', async () => {
        ...
        const operatorList = Promise.withResolvers<void>();
        const page = {
        ...
        const documentSession = {
            pdfDocument,
            leasePage: vi.fn(async () => ({page, release})),
        };
