...
describe('usePdfViewerRenderingRuntime', () => {
    beforeEach(() => {
        ...
        rendererMocks.hasPageVisual.mockReset();
        rendererMocks.isPageCanvasCommitted.mockReset();
        ...
        rendererMocks.usePdfPageRenderer.mockReturnValue({
            ...
            hasPageVisual: rendererMocks.hasPageVisual,
            isPageCanvasCommitted: rendererMocks.isPageCanvasCommitted,
            isPageRendered: rendererMocks.isPageRendered,
        });
    });

    it('does not expose the rendered class until the renderer finalizes the page', () => {
        rendererMocks.isPageCanvasCommitted.mockReturnValue(false);
        const runtime = createRuntime();

        expect(runtime.isPageRenderedForClass(1)).toBe(false);

        rendererMocks.isPageCanvasCommitted.mockReturnValue(true);

        expect(runtime.isPageRenderedForClass(1)).toBe(true);
        expect(rendererMocks.isPageCanvasCommitted).toHaveBeenCalledWith(1);
    });
});
