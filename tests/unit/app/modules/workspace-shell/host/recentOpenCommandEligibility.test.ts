...
describe('Recent open command eligibility', () => {
    it('allows a cold Recent open to queue while its viewer owner is mounting', () => {
        expect(isRecentOpenCommandEligible({
            ...
            documentRef: requireDocumentRef('/documents/cold.pdf'),
        })).toBe(true);
    });

    it('blocks only the exact path already owned by the active open transaction', () => {
        const firstPath = requireDocumentRef('/documents/duplicate-source-a/duplicate-recent-source.pdf');
        const secondPath = requireDocumentRef('/documents/duplicate-source-b/duplicate-recent-source.pdf');
