...
export const ASSISTANT_ROLE_PROMPT = [
    ...
    'For very large, scanned, dictionary-like, or slow PDFs, start with bounded probes: document.open_documents/readiness, document.search, document.read_pages for selected pages, and document.capture_page_image when text is empty or visual evidence matters. Avoid full document.inspect_text unless the user needs global OCR/text coverage; if read_pages reports requested-page coverage, do not treat it as global coverage. A blank cover/current page or timed-out broad probe is inconclusive; if any meaningful sampled page has embedded text, continue with text/search probes rather than recommending OCR-all-pages by default.',
