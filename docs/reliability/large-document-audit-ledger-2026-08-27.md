...
| SRCH-006 | High | product | OCR cancellation is ignored in several paths, including DOCX generation, and the degraded raster guard runs after expensive rendering. | `full-audit.md:73`. | Cancel OCR and DOCX generation before and during raster work; assert no later render or output commit occurs and guard runs before allocation. | lane-f / follow-up-required | `87e73d339` | Focused green; lint, typecheck, 9,696 unit tests, and `build:strict` green; fallow has only two pre-L0 findings; a later combined CodeRabbit pass found that result hashing does not check cancellation per streamed chunk; integration blocked by unrelated baseline E2E failures. | Close and shutdown cancellation plus pre-render raster admission are covered. Add cancellation-aware result hashing. Renderer-initiated DOCX cancellation remains outside lane F ownership. Rerun full integration after the baseline is green. |
| SRCH-007 | Medium | product | OCR page-identity sidecars accept trailing garbage. | `full-audit.md:74`. | Append bytes after a valid page-identity sidecar and assert strict parsing rejects the artifact. | lane-f / follow-up-required | `3bc9fc6e4` | Focused green; lint, typecheck, 9,696 unit tests, and `build:strict` green; fallow has only two pre-L0 findings; a later combined CodeRabbit pass found that lazy identity lookup can stop before tail validation and that delimiter tracking does not reject mismatched bracket types; integration blocked by unrelated baseline E2E failures. | Full scans reject trailing bytes, but lazy readers still need complete structural validation before exposing IDs. Add an in-range corrupt lookup and valid positive controls, then rerun full integration after the baseline and L0 are available. |
