...
import { createPdfNavigationRenderSettledEvent } from '@app/modules/pdf-viewer/runtime/navigation/navigationMachine';
import type { TPdfNavigationEvent } from '@app/modules/pdf-viewer/runtime/navigation/navigationMachine';
import { createPdfCurrentPageAuthority } from '@app/modules/pdf-viewer/runtime/navigation/createPdfCurrentPageAuthority';
import { createPdfNavigationRuntimeForCurrentPage } from '@app/modules/pdf-viewer/runtime/navigation/createPdfNavigationRuntime';
...
import type {
    IApplySnapToMountedPageCommitOptions,
    IPagedRowRenderRequest,
    IUsePdfSinglePageScrollOptions,
} from '@app/modules/pdf-viewer/runtime/navigation/pdfSinglePageScrollTypes';
...
import { createPdfNavigationCommitters } from '@app/modules/pdf-viewer/runtime/navigation/createPdfNavigationCommitters';
import { PDF_SINGLE_PAGE_SCROLL_POLICY } from '@app/modules/pdf-viewer/runtime/navigation/pdfSinglePageScrollPolicy';
import {
    scheduleSinglePageScrollFrame,
    waitForContinuousRenderFrame,
} from '@app/modules/pdf-viewer/runtime/navigation/singlePageScrollFrame';
...
export const usePdfSinglePageScroll = (
    ...
    } = createPdfNavigationCommitters(options);
    const navigationRuntime = createPdfNavigationRuntimeForCurrentPage(
        currentPage,
        emitCurrentPage,
        (page, commitOptions) => commitCurrentPageValue(
            page,
            commitOptions.previousPage,
            commitOptions.transactionId,
        ),
    );
    const currentPageAuthority = createPdfCurrentPageAuthority({
        currentPage,
        navigationRuntime,
        numPages,
        updateCurrentPage,
    });
    const searchNavigationState = navigationRuntime.statusForSource('search');
    const searchNavigationTargetPage = navigationRuntime.targetPageForSource('search');
    const continuousNavigationTargetPage = navigationRuntime.targetPageForSource('continuous');
    const isSearchNavigationLocked = computed(
        () => searchNavigationState.value !== 'idle',
    );
    const pagedNavigationTargetPage = navigationRuntime.targetPageForSource('paged');
    ...
    const navigationEffects = createNavigationSettleEffects({
        ...
        onLayoutReapply: ({
            ...
            logPdfRenderTrace('single-page-continuous-navigation-layout-reapply', {
                runId,
                activeRunId: getActiveContinuousNavigationRunId(),
    ...
    const pagedNavigationAuthority = createPagedNavigationAuthority({
        armHoldWatchdog: navigationEffects.armPagedHoldWatchdog,
        clearHoldWatchdog: navigationEffects.clearPagedHoldWatchdog,
        emitNavigationFeedbackPage,
    ...
    const pagedNavigationHold = pagedNavigationAuthority.hold;
    ...
    function setNavigationFeedbackPage(page: number, reason: string, runId: number) {
        pagedNavigationAuthority.setFeedbackPage(page, reason, runId);
    }

    function clearNavigationFeedbackPage(reason: string, runId?: number) {
        pagedNavigationAuthority.clearFeedbackPage(reason, runId);
    }

    function clearProgrammaticNavigationReleaseTimer() {
        pagedNavigationAuthority.clearProgrammaticReleaseTimer();
    }

    function scheduleProgrammaticNavigationRelease() {
        pagedNavigationAuthority.scheduleProgrammaticRelease({
            isActive: () => isProgrammaticNavigationActive.value,
            isDisposed: () => isDisposed,
            onRelease: maybeReleaseProgrammaticNavigation,
            resolveDelayMs: () => {
                const remainingMs = snapSuppressUntil.value - Date.now();
                return remainingMs > 0
                    ? remainingMs
                    : PDF_SINGLE_PAGE_SCROLL_POLICY.PROGRAMMATIC_NAVIGATION_RELEASE_RETRY_MS;
            },
        });
    }

    function clearPagedNavigationHold(runId?: number) {
        pagedNavigationAuthority.clearHold(runId);
    }
    ...
    function startPagedNavigationHold(runId: number, targetPage: number) {
        ...
        pagedNavigationAuthority.startHold({
            runId,
            targetPage,
            targetStart: targetRange.start,
            targetEnd: targetRange.end,
            readyRetryDelaysMs: PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_READY_RETRY_DELAYS_MS,
            recoveryRenderMs: PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_RECOVERY_RENDER_MS,
            abandonMs: PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_ABANDON_MS,
            stallLogMs: PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_HOLD_STALL_LOG_MS,
            onReadyRetry: () => {
            ...
            onAbandon: () => {
                ...
                pagedNavigationAuthority.clearTargetScrollOptions();
                ...
                currentPageAuthority.syncFromViewportIfIdle(container, 'paged-navigation-abandoned', { requireAuthoritative: true });
    ...
    function isNavigationHoldActiveForPage(pageNumber: number) {
        return pagedNavigationAuthority.isHoldActiveForPage(pageNumber);
    }

    function isNavigationHoldExpiredPage(pageNumber: number) {
        return pagedNavigationAuthority.isHoldExpiredPage(pageNumber);
    }
    ...
    function getActiveSearchNavigationRunId() {
        return navigationRuntime.getTxnForSource('search');
    }

    function getActiveContinuousNavigationRunId() {
        return navigationRuntime.getTxnForSource('continuous');
    }

    function isContinuousNavigationTargetCurrent(runId: number, targetPage: number) {
        return navigationRuntime.isTargetCurrent('continuous', runId, targetPage);
    }
    ...
    function clearContinuousNavigationTarget(runId?: number, pageNumber?: number) {
        const activeRunId = getActiveContinuousNavigationRunId();
        if (runId !== undefined && runId !== activeRunId) {
            return;
        }
        const activeTargetPage = continuousNavigationTargetPage.value;
        if (
            pageNumber !== undefined
            && activeTargetPage !== pageNumber
        ) {
            return;
        }
        ...
        if (activeRunId !== null && activeTargetPage !== null) {
            dispatchNavigationMachine({
                type: 'RENDER_SETTLED',
                txn: activeRunId,
    ...
    function scheduleContinuousNavigationTargetFallbackClear(runId: number, pageNumber: number) {
        navigationEffects.armContinuousTargetFallback(
            PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_NAVIGATION_TARGET_MAX_HOLD_MS,
            () => {
    ...
    function isContinuousNavigationTargetAligned(
        ...
        return Math.abs(container.scrollTop - targetTop) <= PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_NAVIGATION_REAPPLY_EPSILON
        && (
            targetLeft === null
            || Math.abs(container.scrollLeft - targetLeft) <= PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_NAVIGATION_REAPPLY_EPSILON
        );
    }
    ...
    function beginPagedNavigation(
        pageNumber: number,
        holdMs: number = PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_DEFAULT_HOLD_MS,
    ...
    function beginSearchNavigation(
        pageNumber: number,
        holdMs: number = PDF_SINGLE_PAGE_SCROLL_POLICY.SEARCH_NAVIGATION_DEFAULT_HOLD_MS,
    ) {
        navigationEffects.clearSearchSettle();
        cancelContinuousNavigationTarget();
        pagedNavigationPrepareRunId += 1;
        ...
        markProgrammaticNavigation(Math.max(PDF_SINGLE_PAGE_SCROLL_POLICY.SEARCH_NAVIGATION_MIN_HOLD_MS, holdMs));
    }

    function endSearchNavigation(
        settleMs: number = PDF_SINGLE_PAGE_SCROLL_POLICY.SEARCH_NAVIGATION_DEFAULT_SETTLE_MS,
    ) {
        navigationEffects.clearSearchSettle();
        const runId = getActiveSearchNavigationRunId();
        if (runId === null || !navigationRuntime.isTxnCurrent(runId)) {
            return;
        }
        ...
        if (navigationRuntime.isTxnCurrent(runId)) {
            dispatchNavigationMachine({
                type: 'SCROLL_APPLIED',
                txn: runId,
                page: targetPage,
        ...
        markProgrammaticNavigation(Math.max(PDF_SINGLE_PAGE_SCROLL_POLICY.SEARCH_NAVIGATION_DEFAULT_SETTLE_MS, settleMs + PDF_SINGLE_PAGE_SCROLL_POLICY.PROGRAMMATIC_NAVIGATION_RELEASE_RETRY_MS));
        navigationEffects.armSearchSettle(settleMs, () => {
            if (navigationRuntime.isTxnCurrent(runId)) {
                clearNavigationFeedbackPage('search-navigation-finished', runId);
                dispatchNavigationMachine(createPdfNavigationRenderSettledEvent(runId, targetPage));
            }
            maybeReleaseProgrammaticNavigation();
        });
    }
    ...
    function revealSearchNavigationTarget(
        ...
        if (
            ...
            || !navigationRuntime.isTxnCurrent(runId)
        ) {
            return false;
        }
    ...
    }, PDF_SINGLE_PAGE_SCROLL_POLICY.SCROLL_RENDER_DEBOUNCE_MS);
    ...
    function commitPagedNavigationTarget(
        ...
        const rowRange = setVisibleRangeToPageRow(targetPage, runId);
        const didSnap = applySnapToMountedPage(
            targetPage,
            anchor,
            pagedNavigationAuthority.getTargetScrollOptions(),
            { commitCurrentPage: true },
        );
        if (!didSnap) {
            currentPageAuthority.commitNavigationPage('paged', runId, targetPage);
        }
    ...
    function setVisibleRangeToPageRow(pageNumber: number, runId?: number | undefined) {
        if (numPages.value === 0) {
            commitVisibleRangeValue({
                start: 1,
                end: 1,
    ...
    function setPagedNavigationTarget(pageNumber: number, anchor: TPageSnapAnchor | null) {
        ...
        const {
            runId,
            targetPage,
        } = beginPagedNavigation(pageNumber, PDF_SINGLE_PAGE_SCROLL_POLICY.PAGED_NAVIGATION_SETTLE_TIMEOUT_MS, anchor);
        const feedbackState = pagedNavigationAuthority.getFeedbackState();
        if (
    ...
    async function runContinuousNavigationRenderPass(
        ...
        if (
            isDisposed
            || !isContinuousNavigationTargetCurrent(runId, pageNumber)
            || !continuousScroll.value
            || isLoading.value
            || !pdfDocument.value
        ) {
    ...
    function reapplyContinuousNavigationTargetScroll(
        ...
        if (isContinuousNavigationTargetAligned(pageNumber, scrollOptions)) {
            if (currentPageAuthority.canSyncFromViewport()) {
                currentPageAuthority.syncFromViewportIfIdle(viewerContainer.value, 'continuous-navigation-aligned', { requireAuthoritative: true });
            } else {
                currentPageAuthority.commitNavigationPage('continuous', runId, pageNumber);
            }
            return;
        }
        ...
        if (currentPageAuthority.canSyncFromViewport()) {
            currentPageAuthority.syncFromViewportIfIdle(viewerContainer.value, 'continuous-navigation-reapply', { requireAuthoritative: true });
        } else {
            currentPageAuthority.commitNavigationPage('continuous', runId, pageNumber, previous);
        }
    }
    ...
    function queueContinuousNavigationRenderAfterNavigation(
        ...
        logPdfRenderTrace('single-page-queue-continuous-navigation-render', {
            runId,
            delaysMs: [...PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_PROGRAMMATIC_RENDER_SETTLE_DELAYS_MS],
            currentPage: currentPage.value,
            targetPage: pageNumber,
            visibleRange: {
                start: visibleRange.value.start,
        ...
        navigationEffects.armContinuousRender(
            PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_PROGRAMMATIC_RENDER_SETTLE_DELAYS_MS,
            (delayMs) => {
    ...
    function applyContinuousNavigationTargetScroll(
        ...
        updateVisibleRange(viewerContainer.value, numPages.value);
        if (currentPageAuthority.canSyncFromViewport()) {
            currentPageAuthority.syncFromViewportIfIdle(viewerContainer.value, 'continuous-navigation-scroll', { requireAuthoritative: true });
        } else {
            currentPageAuthority.commitActiveNavigationPage(pageNumber, previous);
        }
        return true;
    }
    ...
    function finishContinuousNavigationStart(
        ...
        if (didScroll && isContinuousNavigationTargetCurrent(runId, targetPage)) {
            dispatchNavigationMachine({
                type: 'SCROLL_APPLIED',
                txn: runId,
                page: targetPage,
    ...
    async function finishContinuousNavigationStartAfterMetricHydration(
        ...
        const range = setVisibleRangeToPageRow(targetPage, runId);
        await renderVisiblePages(
            range,
            {
                preserveRenderedPages: true,
    ...
    function applySnapToMountedPage(
        ...
        if (commitOptions?.commitCurrentPage !== false && currentPage.value !== targetPage) {
            const previous = currentPage.value;
            if (!currentPageAuthority.commitActiveNavigationPage(targetPage, previous)) {
                commitCurrentPageValue(targetPage, previous, navigationRuntime.txn.value);
            }
        }
    ...
    function snapToPage(
        ...
        if (!continuousScroll.value) {
            ...
            const didStart = startPreparedPagedNavigationTarget(
                prepareRunId,
                targetPage,
                anchor,
                options,
        ...
        if (currentPage.value !== previous) {
            if (currentPageAuthority.canSyncFromViewport()) {
                currentPageAuthority.commitViewportPageFromSource(currentPage.value, 'single-page-snap-scroll', previous);
            } else if (!currentPageAuthority.commitActiveNavigationPage(targetPage, previous)) {
    ...
    }, PDF_SINGLE_PAGE_SCROLL_POLICY.TALL_PAGE_SNAP_DEBOUNCE_MS);
    ...
    function handleWheel(event: WheelEvent) {
        ...
        if (Math.abs(delta) < PDF_SINGLE_PAGE_SCROLL_POLICY.WHEEL_DELTA_EPSILON) {
            return false;
        }
        ...
        suppressSnapFor(PDF_SINGLE_PAGE_SCROLL_POLICY.WHEEL_FLIP_SNAP_SUPPRESSION_MS);
    ...
    function handleScroll() {
        ...
        if (currentPageAuthority.canSyncFromViewport()) {
            ...
            if (page !== previous) {
                ...
                currentPageAuthority.commitViewportPageFromSource(page, 'single-page-scroll', previous);
            }
        }
        const continuousTargetPage = continuousNavigationTargetPage.value;
        const continuousRunId = getActiveContinuousNavigationRunId();
        if (
            continuousScroll.value
    ...
    function scrollToPage(
        ...
        if (continuousScroll.value) {
            markProgrammaticNavigation(ensurePageMetricsInRange
                ? PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_NAVIGATION_HOLD_WITH_METRICS_MS
                : PDF_SINGLE_PAGE_SCROLL_POLICY.CONTINUOUS_NAVIGATION_HOLD_FALLBACK_MS);
            const targetPage = clamp(pageNumber, 1, numPages.value);
    ...
    return {
        navigationState: navigationRuntime.state,
