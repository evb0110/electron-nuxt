...
async function importKey(bytes: Uint8Array, bits: 128 | 256): Promise<CryptoKey> {
    return crypto.subtle.importKey(
        'raw', bytes, { name: 'AES-CBC', length: bits }, false, ['encrypt', 'decrypt'],
    );
}
...
async function aesDecNoPad(key: CryptoKey, iv: Uint8Array, data: Uint8Array): Promise<Uint8Array> {
    ...
    for (let i = 0; i < AES_BLOCK; i++) padXor[i] = 0x10 ^ lastC[i];

    const fakeEnc = new Uint8Array(
        await crypto.subtle.encrypt(
            { name: 'AES-CBC', iv: new Uint8Array(AES_BLOCK) }, key, padXor,
        ),
    );
...
async function computeHash2B(
    ...
    for (;;) {
        ...
        let s = 0;
        for (let i = 0; i < 16; i++) s += E[i];
        K = await sha(['SHA-256', 'SHA-384', 'SHA-512'][s % 3]!, E);
...
function parseEncryptDict(dict: PDFDict): IEncryptR6 | null {
    ...
    if (!uBytes || uBytes.length < 48 || !ueBytes || ueBytes.length < 32) return null;

    return { R: R.asNumber(), U: uBytes, UE: ueBytes };
}
...
async function deriveFileKey(params: IEncryptR6): Promise<Uint8Array | null> {
    ...
    for (let i = 0; i < 32; i++) {
        if (hash[i] !== params.U[i]) return null;
    }
...
async function decryptContent(
    ...
    if (ct.length === 0 || ct.length % AES_BLOCK !== 0) return null;
...
function hasEncryptMarker(data: Uint8Array): boolean {
    ...
    for (let i = 0; i < data.length; i += chunkSize - marker.length) {
        ...
        if (decoder.decode(data.subarray(i, end)).includes(marker)) return true;
    }
    return false;
}
...
export async function stripPdfEncryption(data: Uint8Array): Promise<Uint8Array> {
    ...
    if (!hasEncryptMarker(data)) return data;
    ...
    if (!encryptRef || !(encryptRef instanceof PDFRef)) return data;

    const encryptDict = doc.context.lookupMaybe(encryptRef, PDFDict);
    if (!encryptDict) return data;

    const params = parseEncryptDict(encryptDict);
    if (!params || params.R !== 6) return data;
