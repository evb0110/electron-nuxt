...
export async function runExternalStressOperator(options: IStressOperatorDriverOptions): Promise<IStressOperatorDriverResult> {
    ...
    const instructions = [
        ...
        'Never select generic Electron by name. If window selection is ambiguous, attach to the exact CDP endpoint with defaultViewport: null instead of using an untracked resolver script.',
        'Open only the Files listed above. Save only those working copies. Leave other app sessions alone.',
        'Use the native Open dialog for the listed files and normal Save for working copies when needed.',
        'If native selection resolves another Electron instance, use the exact CDP endpoint. For file setup, use openPdfInApp or triggerOpenPathInApp from tests/e2e/electron/helpers/viewerCore on that page.',
        'Record any setup fallback in the action log. Perform the requested navigation, editing and tab actions through visible mouse and keyboard controls.',
        ...
        'Reserve the final two minutes for verifying evidence and submitting the report. Do not wait for the runner to exit; it is waiting for your report.',
