...
if (!platform || !arch) {
    throw new Error(`Unsupported platform for pdf-image-combine: ${process.platform}-${process.arch}`);
}
...
const sourcePath = path.join(projectRoot, 'native', 'pdf-image-combine', 'target', 'release', binaryName);
const stageDir = path.join(projectRoot, '.tmp', 'pdf-image-combine', platformArch, 'bin');
