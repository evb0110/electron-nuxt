...
const binaryName = `evb-pdf-image-combine${target.binaryExtension}`;
const cargoArgs = [
    'build',
    '--manifest-path',
...
const sourcePath = path.join(projectRoot, 'native', 'pdf-image-combine', ...target.cargoReleaseDirSegments, binaryName);
const stageDir = path.join(projectRoot, '.tmp', 'pdf-image-combine', target.platformArch, 'bin');
