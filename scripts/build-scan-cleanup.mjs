...
const binaryName = `evb-scan-cleanup${target.binaryExtension}`;
const cargoArgs = [
    'build',
    '--manifest-path',
    'native/scan-cleanup/Cargo.toml',
    '--release',
    '--locked',
    ...target.cargoTargetArgs,
];
const cargoTargetDirectory = resolveCargoTargetDirectory({
    manifestPath: 'native/scan-cleanup/Cargo.toml',
    projectRoot,
});
...
const stageDir = path.join(projectRoot, '.tmp', 'scan-cleanup', target.platformArch, 'bin');
